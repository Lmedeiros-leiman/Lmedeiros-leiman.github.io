...
export const HistoryItem : React.FC = ({children}) => {
  ...
  return (<div className="flex justify-between w-full py-1 px-5 hover:bg-white/10">
    <div className="flex gap-5 items-center ">
      <span><FaHistory /> </span>
      <span>{children}</span>
    </div>
    <aside className="text-[#3ea6ff] text-sm hover:underline">
      Remove
    </aside>
  ...
  return (<>
    <div className="flex flex-grow justify-center px-4 ms-10">
      <form className="w-full relative ms-8 flex  ">
        ...
        <div className={ (showDropdown ? "block" : "hidden") + " mr-2 w-8 flex  h-full items-center"}>
          <span >
            <SearchIcon />
          </span>
        </div>
        ...
        >
          <ul>
          {Array.from({length: 10}, (_,i) => <HistoryItem key={i}>{("Lorem ipsum dolor sit amet consectetur adipiscing elit").split(' ').slice(0, Math.floor(Math.random() * 9) + 1).join(' ')}</HistoryItem>)}
          </ul>
        </div>
      </form>
